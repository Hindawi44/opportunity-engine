"""Official-domain decision-support feed for Stock-Hurt wholesale stock.

The collector surfaces specific products, catalogue pages, and pallet-auction
signals from Stock-Hurt. Missing commercial details and large lots remain visible
for human calculation and decision; no commercial action is automated.
"""
...
QUERIES = (
    'site:stockhurt.com/en/product/ (clothing OR clothes OR jackets OR dresses) '
    '(wholesale OR outlet OR stock OR pallet OR package OR kg OR "grade A" OR "grade B")',
    'site:stockhurt.com/en/ (licytacje OR auction OR auctions OR shop) '
    '(clothing OR clothes OR apparel OR pallet OR package OR box OR wholesale)',
)
...
_GRADE_RE = re.compile(r"\bgrade\s*[:\-]?\s*(?P<grade>[ABC])\b", re.IGNORECASE)
_BRAND_FIELD_RE = re.compile(
    r"\bbrand\s*[:\-]\s*(?P<brand>[^|;\n.]{2,100})",
    re.IGNORECASE,
)
_MINIMUM_WEIGHT_RE = re.compile(
    r"\bminimum\s+(?:weight|quantity)\s*(?:of|is|:|-)?\s*"
    r"(?P<amount>\d{1,9}(?:[\s.,]\d{3})*(?:[.,]\d{1,2})?)\s*"
    r"(?P<unit>pcs?|pieces?|units?|items?|pairs?|sets?|kg|kilograms?)",
    re.IGNORECASE,
)
_UNIT_HINT_RE = re.compile(
    r"(?:\(|\bunit\s*[:\-]\s*)(?P<unit>kg|pcs?|pieces?|units?)(?:\)|\b)",
    re.IGNORECASE,
)


def _extract_minimum_order(
    text: str,
) -> tuple[float | None, str | None, tuple[int, int] | None]:
    amount, unit, span = _extract_moq(text)
    if amount is not None:
        return amount, unit, span
    match = _MINIMUM_WEIGHT_RE.search(text)
    if not match:
...
def _extract_source_brands(text: str) -> list[str]:
    brands = _extract_brands(text)
    field = _BRAND_FIELD_RE.search(text)
    if field:
        brands.extend(
            item.strip(" ,.-")[:80]
            for item in re.split(r"[,/]", field.group("brand"))
            if len(item.strip(" ,.-")) >= 2
        )
    brands.extend(term.title() for term in _matched_terms(text, _KNOWN_BRAND_TERMS))
    return sorted({brand for brand in brands if brand})[:15]


def _extract_grade(text: str) -> str | None:
    match = _GRADE_RE.search(text)
    return match.group("grade").upper() if match else None
...
def _page_role(path: str, text: str) -> str:
    if "/licytacje" in path or _matched_terms(text, _AUCTION_TERMS):
        return "PALLET_AUCTION_SIGNAL"
    if "/product/" in path:
        return "SPECIFIC_STOCK_OFFER"
    return "STOCK_CATALOGUE_SIGNAL"
...
def _candidate_from_hit(
    ...
    *,
    ...
    specific_offer = role in {"SPECIFIC_STOCK_OFFER", "PALLET_AUCTION_SIGNAL"}
    opportunity_state = (
        "B2B_LEAD_REQUIRES_VERIFICATION"
        if specific_offer and listing_status != "OUT_OF_STOCK" and len(missing_information) <= 3
    ...
    score += 7 if brands else 0
    score += 7 if manifest_terms else 0
    score += 6 if authenticity_terms else 0
    score += 4 if condition_terms or grade else 0
    score += 3 if shipping_terms else 0
    score += 3 if specific_offer else 0
    score += 2 if auction_terms else 0
    score -= 10 if listing_status == "OUT_OF_STOCK" else 0
    score = max(0, min(100, score))

    candidate_id = "stockhurt-b2b:" + sha256(
    ...
    return {
        ...
        "qualification_blockers": [
            "VERIFY_OFFER_ACTIVE_AND_CURRENT_AVAILABILITY",
            "VERIFY_COMPANY_LEGAL_IDENTITY_AND_PAYMENT_DETAILS",
            "VERIFY_COMPLETE_CONTENTS_OR_STOCK_LIST",
            "VERIFY_GRADE_DEFECTS_AND_SAMPLE QUALITY",
...
def stockhurt_candidate_from_hit(
    hit: SearchHit,
    *,
    observed_at: datetime,
) -> dict[str, Any] | None:
...
def collect_stockhurt_b2b_feed(
    *,
    ...
    return {
        ...
        "feed_family": FEED_FAMILY,
        "purpose": "OFFICIAL_SOURCE_WHOLESALE_STOCK_AND_AUCTION_DECISION_SUPPORT",
        "approved_official_domains": list(APPROVED_DOMAINS),
        "source_count": 1,
        ...
        "operator_rule": "SURFACE_ALL_SERIOUS_STOCK_AND_AUCTION_SIGNALS_CALCULATE_AND_LEAVE_DECISION_TO_HUMAN",
