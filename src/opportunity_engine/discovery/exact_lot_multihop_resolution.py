...
commercially-scoped breadth-first walk:

    verified root -> hub/page -> collection -> canonical product

It is deliberately not a general crawler. Only public HTTPS links on the same
origin and in an allowlisted commercial URL role are considered. Blogs, legal,
account, cart, checkout, contact and other side paths are excluded. Depth, root,
link and page-fetch budgets are hard bounded. Gateway pages never receive
Exact-Lot credit; only directly fetched pages that pass the strict child-page
classifier can become Exact-Lot candidates.
"""
...
from opportunity_engine.discovery.exact_lot_child_link_resolution import (
    ...
    _compact,
    _eligible_parent,
    _looks_canonical_product_detail_url,
    fetch_public_html,
)
...
def _commercial_url_role(url: str) -> str | None:
    """Return a bounded navigable commercial role, never a general-web role."""
    path = _path(url).rstrip("/") or "/"
    if any(path == prefix or path.startswith(prefix + "/") for prefix in _EXCLUDED_PATH_PREFIXES):
        return None
    if _looks_canonical_product_detail_url(url):
...
def _base(
    *,
    provider: str,
    max_root_parents: int,
    max_navigation_depth: int,
    max_links_per_page: int,
    max_navigation_page_fetches: int,
) -> dict[str, Any]:
    return {
...
def resolve_exact_lot_multihop(
    ...
    *,
    ...
    if not 1 <= max_navigation_depth <= MAX_NAVIGATION_DEPTH:
        raise ValueError(
            f"max_navigation_depth must be between 1 and {MAX_NAVIGATION_DEPTH}"
        )
    ...
    if not 1 <= max_navigation_page_fetches <= MAX_NAVIGATION_PAGE_FETCHES:
        raise ValueError(
            "max_navigation_page_fetches must be between 1 and "
            f"{MAX_NAVIGATION_PAGE_FETCHES}"
        )
    ...
    for root in roots:
        ...
        while queue:
            ...
            if _host(final_url) != root_host or _commercial_url_role(final_url) is None:
                navigation_results.append(
                    {
                        **node,
            ...
            row = {
                **node,
                ...
                "navigation_role": _commercial_url_role(final_url),
