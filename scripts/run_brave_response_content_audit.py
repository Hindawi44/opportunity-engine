#!/usr/bin/env python3
"""Run V2.7.2.4.5 Brave response content audit."""
...
from opportunity_engine.brave_response_content_audit import (
    BraveResponseContentRecord,
    ResponseContentAuditedBraveProvider,
    summarize_content,
)
...
def _candidate_record(
    *, candidate: Any, result: Any, requests: list[BraveResponseContentRecord]
) -> dict[str, Any]:
    return {
        ...
        "total_parser_results": sum(item.parser_results_count for item in requests),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit Brave raw JSON response structure")
    parser.add_argument("dataset", nargs="?", default="data/todays_opportunities.json")
    parser.add_argument(
        "--output",
        default="data/validation/v2.7.2.4.5-brave-response-content-audit.json",
    )
    parser.add_argument(
        "--raw-dir", default="data/validation/v2.7.2.4.5-brave-raw-responses"
    )
    ...
    loop = build_loop()
    audited_provider = ResponseContentAuditedBraveProvider(
        loop.search_provider, raw_dir=args.raw_dir
    )
    ...
    for candidate in candidate_report.records:
        ...
        candidate_records.append(
            _candidate_record(candidate=candidate, result=result, requests=requests)
        )

    report = {
        "schema_version": "2.7.2.4.5",
        "audit_scope": "Brave raw response content and parser-path comparison only",
        "expected_parser_path": "web.results",
        "selected_candidates": candidate_report.selected_count,
        "audited_candidates": len(candidate_records),
        "summary": summarize_content(audited_provider.records),
