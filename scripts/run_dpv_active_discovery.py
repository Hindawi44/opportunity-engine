#!/usr/bin/env python3
"""Watch active public Deutsche Pfandverwertung catalogs for clothing inventory."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default="artifacts/germany-dpv-active",
    )
    parser.add_argument("--index-url", default=DEFAULT_DPV_INDEX_URL)
    parser.add_argument(
        "--catalog-limit",
        type=int,
        default=DEFAULT_ACTIVE_CATALOG_LIMIT,
    )
    parser.add_argument(
        "--catalog-page-limit",
        type=int,
        default=DEFAULT_CATALOG_PAGE_LIMIT,
    )
    parser.add_argument(
    ...
    if not 1 <= args.catalog_limit <= 25:
        raise SystemExit("--catalog-limit must be between 1 and 25")
    if not 1 <= args.catalog_page_limit <= 200:
        raise SystemExit("--catalog-page-limit must be between 1 and 200")
    if not 0 <= args.item_verification_limit <= 100:
        raise SystemExit("--item-verification-limit must be between 0 and 100")
    ...
    active = run_dpv_active_clothing_watch(
        args.index_url,
    ...
    report["source_mode"] = "DPV_ACTIVE_WATCH"
    report["source_target"] = "DEUTSCHE_PFANDVERWERTUNG_ACTIVE_AUCTIONS"
    report["query_pack"] = "DPV_ACTIVE_INDEX_V1"
    ...
    diagnostics_path = output_dir / "dpv-active-diagnostics.json"
    ...
    paths["dpv_active_diagnostics"] = diagnostics_path
    ...
    print(f"Auction entries discovered: {diagnostics['auction_entries_discovered']}")
    print(f"Active catalog entries: {diagnostics['active_catalog_entries_discovered']}")
    print(f"Selected catalogs: {diagnostics['selected_catalog_count']}")
    print(f"Successful catalogs: {diagnostics['successful_catalog_count']}")
    print(f"Clothing catalogs: {diagnostics['clothing_catalog_count']}")
    ...
    print(f"Verified bulk lots: {diagnostics['verified_bulk_lot_count']}")
    ...
    if report.get("status") != "PASS":
        print("DPV watch completed with catalog or item errors", file=sys.stderr)
        return 2
    return 0
