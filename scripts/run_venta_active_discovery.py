#!/usr/bin/env python3
"""Watch active public VENTA catalogs for explicit clothing inventory."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        default="artifacts/germany-venta-active",
    )
    parser.add_argument("--index-url", default=DEFAULT_VENTA_INDEX_URL)
    parser.add_argument(
        "--catalog-limit",
        type=int,
    ...
    report["source_mode"] = "VENTA_ACTIVE_WATCH"
    report["source_target"] = "VENTA_ACTIVE_AUCTIONS"
    report["query_pack"] = "VENTA_ACTIVE_INDEX_V1"
    ...
    diagnostics_path = output_dir / "venta-active-diagnostics.json"
    ...
    paths["venta_active_diagnostics"] = diagnostics_path
    ...
    print(
        ...
        f"{diagnostics['active_catalog_entries_discovered']}"
    )
    ...
    if report.get("status") != "PASS":
        print("VENTA watch completed with catalog errors", file=sys.stderr)
        return 2
    return 0
