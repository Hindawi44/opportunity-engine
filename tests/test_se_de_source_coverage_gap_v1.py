...
def test_source_gap_radar_runs_nine_requests_and_keeps_safety_gates(tmp_path: Path) -> None:
    ...
    fixtures = {
        ...
        "site:auktion.kronofogden.se": SearchHit(
            title="Kläder & Skor - Webauktion",
            url="https://auktion.kronofogden.se/auk/w.ObjectList?inCategoryId=1",
            description="Varuparti och konkurslager med kläder säljs på auktion.",
            provider="Brave Search",
        ),
        "site:psauction.se": SearchHit(
    ...
    class FakeProvider:
        ...
        def search(self, query: str, *, count: int = 10):
            ...
            for marker, hit in fixtures.items():
                if marker in query:
                    return [hit]
            return []
...
def test_daily_wrapper_still_runs_source_gap_before_core() -> None:
