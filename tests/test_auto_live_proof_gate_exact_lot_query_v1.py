...
EXACT_LOT_CHECKPOINT_RUNNER = "scripts/run_exa_exact_lot_checkpoint.py"
...
def test_existing_exact_lot_checkpoint_runner_remains_covered() -> None:
    assert _matches_gate(EXACT_LOT_CHECKPOINT_RUNNER)


def test_near_match_and_unrelated_document_do_not_trigger_auto_live_proof() -> None:
    assert not _matches_gate(
        "src/opportunity_engine/discovery/exa_exact_lot_shadow_hunt_notes.py"
    )
