...
from opportunity_engine.discovery.netherlands_case_memory_adapter import (
    ENGINE_VERSION,
    adapt_netherlands_signal_to_entity_memory,
...
def _insolvency_signal(*, observed_at: datetime, url: str, title: str) -> dict:
    signal = netherlands_signal_from_hit(
        SearchHit(
            ...
            url=url,
            description=(
                "Kledingwinkel in faillissement. Curator onderzoekt de handelsvoorraad, "
                "mode, textiel en mogelijke verkoop."
            ),
            provider="Fake Brave",
        ),
...
def _tulip_signal(*, observed_at: datetime, url: str) -> dict:
...
def test_adapter_requires_a_concrete_dutch_company_identity() -> None:
    concrete = _tulip_signal(
        observed_at=DAY_1,
        url="https://news.example.nl/tulip-faillissement",
    )
    adapted, reason = adapt_netherlands_signal_to_entity_memory(concrete)

    assert reason is None
    assert adapted is not None
    assert adapted["company_name"] == "Tulip Mode B.V"
    ...
    assert adapted["metadata"]["entity_key"] == "tulip mode"
    assert adapted["metadata"]["entity_label"] == "Tulip Mode B.V"
    assert adapted["metadata"]["promotion_to_opportunity_allowed"] is False

    generic = _insolvency_signal(
        observed_at=DAY_1,
        url="https://news.example.nl/faillissement-mode",
        title="Faillissement in de kleding- en modesector",
    )
    rejected, rejection_reason = adapt_netherlands_signal_to_entity_memory(generic)
    assert rejected is None
    assert rejection_reason == "NO_EXPLICIT_DUTCH_ENTITY"


def test_two_days_for_same_company_build_one_stable_case() -> None:
    first = _tulip_signal(
        observed_at=DAY_1,
        url="https://source-one.example.nl/tulip-faillissement",
    )
    second = _tulip_signal(
        observed_at=DAY_2,
        url="https://source-two.example.nl/tulip-voorraad",
    )

    report = build_netherlands_case_memory_adapter([first, second], observed_at=DAY_2)

    assert report["adapted_entity_signal_count"] == 2
    assert report["persistent_case_count"] == 1
    case = report["cases"][0]
    assert case["entity_key"] == "tulip mode"
    ...
    first_only = build_netherlands_case_memory_adapter([first], observed_at=DAY_1)
    assert first_only["cases"][0]["case_id"] == case["case_id"]


def test_netherlands_follow_up_rotates_through_five_inventory_hunt_stages() -> None:
    signal = _tulip_signal(
        observed_at=DAY_1,
        url="https://news.example.nl/tulip-faillissement",
    )
    ...
    for day in (16, 17, 18, 19, 20):
        report = build_netherlands_case_memory_adapter(
            [signal],
    ...
    assert "handelsvoorraad" in queries[0]
    assert "veiling" in queries[1]
    assert "opheffingsuitverkoop" in queries[2]
    assert "curator" in queries[3]
    assert "kavel" in queries[4]
...
def test_news_today_auction_tomorrow_stays_on_same_case_and_surfaces_lead(
    tmp_path: Path,
) -> None:
    first = _tulip_signal(
        observed_at=DAY_1,
    ...
    second = _tulip_signal(
        observed_at=DAY_2,
        url="https://news-two.example.nl/tulip-curator",
    )
    provider = _FakeProvider(
        [SearchHit(
            title="Tulip Mode B.V. - veiling handelsvoorraad kleding",
            url="https://veiling.example.nl/tulip-kavel-800",
            description=(
                "Faillissementsveiling met kavel van 800 kledingstukken. "
                "Voorraad in openbare verkoop; bieden mogelijk."
            ),
            provider="Fake Brave",
        )]
    )
    ...
    assert case["last_seen"] == DAY_2.isoformat()
    assert case["entity_source_signal_count"] == 2
    assert day_two["persistence"]["loaded_netherlands_entity_signal_count"] == 2

    assert provider.calls
    assert "veiling" in provider.calls[0][0].casefold()
    ...
    assert row["follow_up_stage"] == "VEILING"
    assert row["follow_up_state"] == "COMMERCIAL_LEAD_REQUIRES_SOURCE_VERIFICATION"
    lead = row["leads"][0]
    assert lead["source_url"] == "https://veiling.example.nl/tulip-kavel-800"
