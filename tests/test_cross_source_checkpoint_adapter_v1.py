...
def test_daily_checkpoint_keeps_norway_cross_source_with_thirteen_bounded_sources() -> None:
    ...
    assert 'len(report.get("sources") or []) != 13' in text
    assert 'sum((report.get("source_execution_counts") or {}).values()) != 13' in text
